...
BEGIN
  ...
  tecnicos_validos AS (
    SELECT DISTINCT up.funcionario_id AS id
    FROM user_profiles up
    JOIN perfis_acesso pa ON pa.id = up.perfil_id
    WHERE up.empresa_id = v_empresa
      AND up.funcionario_id IS NOT NULL
      AND lower(unaccent(pa.nome_perfil)) LIKE '%tecnico%'
    UNION
    SELECT DISTINCT s.tecnico_id AS id
    FROM os_servicos s
    WHERE s.empresa_id = v_empresa
      AND s.tecnico_id IS NOT NULL
  ),
  agregado AS (
    ...
    FROM funcionarios f
    JOIN tecnicos_validos tv ON tv.id = f.id
