...
-- 1) Normalize status catalog
UPDATE public.atacado_status_aparelho
SET nome = 'estoque'
WHERE lower(trim(nome)) IN ('stoque','estoque')
  AND categoria = 'em_estoque';

-- 2) Normalize existing aparelhos
UPDATE public.atacado_aparelhos
SET status = 'estoque'
WHERE lower(trim(status)) IN ('stoque','estoque');

-- 3) Harden RPC: canonicalize status on insert
...
DECLARE
  ...
  v_count int := 0;
  v_imei_atual text;
  v_status_in text := COALESCE(NULLIF(trim(p_payload->>'status'),''),'estoque');
  v_status_cat text;
  v_status_final text;
BEGIN
  IF v_emp IS NULL THEN RETURN jsonb_build_object('success',false,'error','sem empresa'); END IF;

  -- Canonicaliza status: se for categoria em_estoque (do catálogo da empresa), força 'estoque'
  SELECT categoria INTO v_status_cat
  FROM public.atacado_status_aparelho
  WHERE empresa_id = v_emp AND lower(nome) = lower(v_status_in)
  LIMIT 1;
  v_status_final := CASE
    WHEN v_status_cat = 'em_estoque' OR lower(v_status_in) IN ('stoque','estoque') THEN 'estoque'
    ELSE lower(v_status_in)
  END;
  ...
  FOR v_unit IN SELECT * FROM jsonb_array_elements(p_payload->'unidades') LOOP
    ...
    BEGIN
      INSERT INTO public.atacado_aparelhos (
        ...
        v_unit->>'imei1', v_unit->>'imei2', v_status_final, 1,
  ...
  RETURN jsonb_build_object('success',true,'invoice_id',v_inv,'qtd',v_count);
END;
$function$;
