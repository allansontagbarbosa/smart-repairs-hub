...
BEGIN
  ...
  SELECT COALESCE(SUM(COALESCE(valor_total, valor, 0)), 0),
    ...
    WHERE empresa_id = v_empresa_id AND deleted_at IS NULL
      ...
      AND data_conclusao <= v_fim_mes_passado;
  ...
  SELECT COALESCE(SUM(COALESCE(valor_total, valor, 0)), 0),
    ...
    WHERE empresa_id = v_empresa_id AND deleted_at IS NULL
      AND status IN ('entregue', 'pronto')
      AND data_conclusao >= v_inicio_mes_passado
      AND data_conclusao <= v_fim_periodo_mes_passado;

  SELECT COALESCE(SUM(c.valor), 0)
    INTO v_com_periodo_mp
    FROM public.comissoes c
    JOIN public.os_servicos s ON s.id = c.servico_id
    ...
    WHERE c.empresa_id = v_empresa_id
      ...
      AND o.data_conclusao <= v_fim_periodo_mes_passado;

  -- Rateio proporcional das despesas
...
BEGIN
  ...
  -- TESTE 9: LL_mes_passado = Fat - Peca - Terc - Desp - Com (mês passado)
  ...
  -- TESTE 10: Meu_valor_mes_passado = Distribuivel_mp × meu_pct
