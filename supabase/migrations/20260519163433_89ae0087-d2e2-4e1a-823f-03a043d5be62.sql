CREATE TABLE IF NOT EXISTS public.socio_metas (
  ...
  valor_acumulado_centavos BIGINT NOT NULL DEFAULT 0,
  data_alvo DATE,
  icone TEXT DEFAULT '🎯',
...
CREATE INDEX IF NOT EXISTS idx_socio_metas_user ON public.socio_metas(user_id) WHERE ativo = true;
CREATE INDEX IF NOT EXISTS idx_socio_metas_empresa ON public.socio_metas(empresa_id);

ALTER TABLE public.socio_metas ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "socio_metas_proprio_select" ON public.socio_metas;
DROP POLICY IF EXISTS "socio_metas_proprio_insert" ON public.socio_metas;
DROP POLICY IF EXISTS "socio_metas_proprio_update" ON public.socio_metas;
DROP POLICY IF EXISTS "socio_metas_proprio_delete" ON public.socio_metas;

CREATE POLICY "socio_metas_proprio_select" ON public.socio_metas FOR SELECT TO authenticated
  USING (user_id = auth.uid() AND empresa_id = public.get_my_empresa_id());
CREATE POLICY "socio_metas_proprio_insert" ON public.socio_metas FOR INSERT TO authenticated
  WITH CHECK (user_id = auth.uid() AND empresa_id = public.get_my_empresa_id());
CREATE POLICY "socio_metas_proprio_update" ON public.socio_metas FOR UPDATE TO authenticated
  USING (user_id = auth.uid() AND empresa_id = public.get_my_empresa_id());
CREATE POLICY "socio_metas_proprio_delete" ON public.socio_metas FOR DELETE TO authenticated
  USING (user_id = auth.uid() AND empresa_id = public.get_my_empresa_id());

DROP TRIGGER IF EXISTS update_socio_metas_updated_at ON public.socio_metas;
CREATE TRIGGER update_socio_metas_updated_at
  BEFORE UPDATE ON public.socio_metas
...
BEGIN
  ...
  v_ll_mes_passado := v_ll_mes_passado
    - COALESCE((SELECT SUM(valor) FROM public.contas_a_pagar
        WHERE empresa_id = v_empresa_id AND status = 'pago'
...
REVOKE ALL ON FUNCTION public.get_painel_socio_v1() FROM PUBLIC;
