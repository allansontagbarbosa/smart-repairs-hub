...
WHERE deleted_at IS NULL
  ...
  AND LOWER(TRIM(cargo)) IN ('tecnico', 'técnico');

UPDATE public.funcionarios
SET cargo = TRIM(cargo)
WHERE deleted_at IS NULL AND cargo IS NOT NULL AND cargo <> TRIM(cargo);

-- Drop old signature and recreate get_painel_socio_v1 with p_meses_historico parameter
DROP FUNCTION IF EXISTS public.get_painel_socio_v1();
DROP FUNCTION IF EXISTS public.get_painel_socio_v1(int);
...
DECLARE
  ...
  v_fat_parcial numeric;
  v_peca_parcial numeric;
  v_com_parcial numeric;
  v_despesas_parcial numeric;
  v_ll_parcial numeric;
  v_distrib_parcial numeric;
  v_reserva_pct numeric;
  v_reserva_val_parcial numeric;
  v_meu_valor_parcial numeric;

  v_fat_prev numeric;
  v_peca_prev numeric;
  v_com_prev numeric;
  v_despesas_completo numeric;
  v_ll_prev numeric;
  ...
  v_caixa numeric := 0;
  v_capital_giro numeric := 0;
  v_dias_runway int := 0;
  v_pagar_30d numeric := 0;
...
BEGIN
  ...
  IF v_socio_id IS NULL THEN
    SELECT id, percentual_participacao INTO v_socio_id, v_meu_percentual
      FROM socios WHERE empresa_id = v_empresa_id AND ativo = true AND deleted_at IS NULL
      LIMIT 1;
  END IF;
  ...
  WITH base AS (
    SELECT
      (s->>'id')::uuid AS id,
      s->>'nome' AS nome,
      (s->>'percentual')::numeric AS percentual,
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'variacao_pct', CASE
      WHEN COALESCE(mp.valor_mp, 0) > 0
        THEN round(((b.valor - mp.valor_mp) / mp.valor_mp * 100)::numeric, 1)
      WHEN b.valor > 0 THEN 100
      ELSE 0
    END,
  ...
  INTO v_socios_lista
  FROM base b
  LEFT JOIN mp ON mp.id = b.id
  LEFT JOIN ytd ON ytd.id = b.id;

  -- Histórico p_meses_historico
  WITH meses AS (
    SELECT
  ...
  dre_mes AS (
    SELECT
      m.mes_inicio, m.mes_fim,
      CASE WHEN m.mes_inicio = v_inicio_mes
        THEN public.get_dre_periodo(m.mes_inicio, current_date, v_empresa_id)
        ELSE public.get_dre_periodo(m.mes_inicio, m.mes_fim, v_empresa_id)
      END AS dre
    FROM meses m
  )
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'meu_valor', COALESCE((
      SELECT (s->>'valor')::numeric
      FROM jsonb_array_elements(d.dre->'distribuicao'->'socios') s
      WHERE (s->>'id')::uuid = v_socio_id
    ), 0)
  ...
  -- Funcionários ROI (com filtro "teste" e ROI computado)
  WITH funcs AS (
    SELECT
      f.id, f.nome, COALESCE(f.cargo, f.funcao, '') AS cargo,
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'receita_centavos', receita_centavos,
    'roi', CASE WHEN custo_centavos > 0
                THEN round((receita_centavos::numeric / custo_centavos)::numeric, 2)
                ELSE NULL END,
    'status', CASE
  ...
  -- Saúde: caixa, runway, capital de giro
  SELECT
  ...
  WHERE empresa_id = v_empresa_id
    AND deleted_at IS NULL
    AND status IN ('pendente','pendente_parcial')
    AND data_vencimento BETWEEN current_date AND current_date + interval '30 days';

  v_capital_giro := v_caixa - v_pagar_30d;
  v_dias_runway := CASE
    WHEN v_gastos_fixos_compl > 0 THEN GREATEST(0, FLOOR(v_caixa / (v_gastos_fixos_compl / 30.0))::int)
    ELSE 0
  END;
  ...
  v_resultado := jsonb_build_object(
    ...
    'mes_atual', jsonb_build_object(
      ...
      'reserva_val', v_reserva_val_parcial,
...
DECLARE
  ...
  v_fim_mes_passado date := (date_trunc('month', current_date) - interval '1 day')::date;
  v_ll_atual numeric;
  v_ll_passado numeric;
  v_fat_atual numeric;
  v_var_margem numeric;
BEGIN
  ...
  -- Contas vencidas (inadimplência interna)
  SELECT COUNT(*), COALESCE(SUM(valor), 0) INTO v_qtd, v_total
  FROM contas_a_pagar
  WHERE empresa_id = v_empresa_id
    AND deleted_at IS NULL
...
GRANT EXECUTE ON FUNCTION public.get_alertas_socio() TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_painel_socio_v1(int) TO authenticated;
