...
BEGIN
  ...
  IF v_dias_passados > 0 THEN
    ...
    v_fechamento_previsto := v_meu_valor_parcial * v_fator_projecao;
  ELSE
    v_fator_projecao := 1;
  ...
  terc AS (
    SELECT date_trunc('month', ord.data_conclusao)::date AS mes,
      ...
      FROM public.os_servicos oss
      JOIN public.ordens_de_servico ord ON ord.id = oss.ordem_id
      WHERE oss.empresa_id = v_empresa_id AND oss.motivo_sem_tecnico = 'terceirizado'
        AND ord.status IN ('entregue', 'pronto')
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', f.id, 'nome', f.nome, 'cargo', COALESCE(f.cargo, '—'),
      ...
      'roi', CASE WHEN COALESCE(f.salario_centavos, 0) = 0 THEN NULL
  ...
  WHERE f.empresa_id = v_empresa_id AND f.ativo = true AND f.deleted_at IS NULL
    ...
    AND (LOWER(f.cargo) LIKE '%tec%' OR LOWER(f.cargo) LIKE '%téc%')
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      'data_alvo', m.data_alvo, 'icone', m.icone, 'cor', m.cor,
  ...
  v_saude_caixa := jsonb_build_object(
    'inadimplencia_centavos', ROUND(COALESCE((SELECT SUM(valor) FROM public.contas_a_pagar
        WHERE empresa_id = v_empresa_id AND status != 'paga' AND deleted_at IS NULL
          AND data_vencimento < current_date), 0) * 100)::bigint,
    'inadimplencia_qtd', COALESCE((SELECT COUNT(*) FROM public.contas_a_pagar
        WHERE empresa_id = v_empresa_id AND status != 'paga' AND deleted_at IS NULL
          AND data_vencimento < current_date), 0),
    'inadimplencia_dias_max', COALESCE((SELECT MAX(current_date - data_vencimento) FROM public.contas_a_pagar
        WHERE empresa_id = v_empresa_id AND status != 'paga' AND deleted_at IS NULL
          AND data_vencimento < current_date), 0),
    'gastos_fixos_mes_centavos', ROUND(COALESCE((SELECT AVG(soma) FROM (
          SELECT date_trunc('month', data_pagamento) AS mes, SUM(valor) AS soma
            FROM public.contas_a_pagar
            WHERE empresa_id = v_empresa_id AND status = 'paga' AND deleted_at IS NULL
              AND data_pagamento >= current_date - interval '3 months'
            GROUP BY 1) sub), 0) * 100)::bigint
  );

  RETURN jsonb_build_object(
    'sucesso', true,
    'gerado_em', now(),
    'socio', jsonb_build_object('id', v_socio.id, 'nome', v_socio.nome, 'percentual', v_socio.percentual_participacao),
    'periodo', jsonb_build_object(
      'inicio_mes', v_inicio_mes, 'fim_mes', v_fim_mes, 'hoje', current_date,
    ...
    'mes_atual', jsonb_build_object(
      ...
      'confiabilidade_projecao', CASE
        ...
        WHEN v_dias_passados < 15 THEN 'média'
        ELSE 'alta'
      END
    ),
    'mes_passado', jsonb_build_object('lucro_liquido', v_ll_mes_passado, 'meu_valor', v_meu_valor_mes_passado),
    'variacao_mes', CASE
