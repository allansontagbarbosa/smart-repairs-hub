...
BEGIN
  ...
  IF v_empresa_id IS NULL THEN
    -- fallback para uso em testes SQL (sem auth)
  ...
  -- Mês atual = DRE parcial (fonte da verdade)
  ...
  -- Previsão (variáveis projetadas × despesas mês inteiro)
  ...
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'id', s->>'id',
      'nome', s->>'nome',
      'percentual', (s->>'percentual')::numeric,
      'valor', (s->>'valor')::numeric,
      'valor_estimado', (s->>'valor')::numeric,
      'eh_voce', (s->>'id')::uuid = v_socio_id
    )
  ), '[]'::jsonb)
  INTO v_socios_lista
  FROM jsonb_array_elements(COALESCE(v_dre_parcial->'distribuicao'->'socios', '[]'::jsonb)) s;

  -- Histórico 6 meses (via DRE por mês - consistente)
  WITH meses AS (
    SELECT
  ...
  dre_mes AS (
    SELECT
      ...
      m.mes_fim,
      CASE
        WHEN m.mes_inicio = v_inicio_mes
