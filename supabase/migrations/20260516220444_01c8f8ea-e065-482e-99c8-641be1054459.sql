...
CREATE TABLE IF NOT EXISTS public.tecnico_metas_mensais (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  empresa_id UUID NOT NULL,
  funcionario_id UUID NOT NULL,
  ano INTEGER NOT NULL,
  mes INTEGER NOT NULL CHECK (mes BETWEEN 1 AND 12),
  meta_qtd_os INTEGER NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS idx_tecnico_metas_empresa_ano_mes
  ON public.tecnico_metas_mensais (empresa_id, ano, mes)
  WHERE deleted_at IS NULL;

ALTER TABLE public.tecnico_metas_mensais ENABLE ROW LEVEL SECURITY;

CREATE POLICY metas_select ON public.tecnico_metas_mensais
  FOR SELECT TO authenticated
  USING (empresa_id = public.get_my_empresa_id());

CREATE POLICY metas_admin_all ON public.tecnico_metas_mensais
...
CREATE TABLE IF NOT EXISTS public.tecnico_sessoes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  funcionario_id UUID NOT NULL,
  empresa_id UUID NOT NULL,
  status TEXT NOT NULL CHECK (status IN ('trabalhando','pausa','almoco','encerrado')),
...
CREATE INDEX IF NOT EXISTS idx_tecnico_sessoes_funcionario_atual
  ON public.tecnico_sessoes (funcionario_id)
  WHERE encerrado_em IS NULL;

ALTER TABLE public.tecnico_sessoes ENABLE ROW LEVEL SECURITY;

CREATE POLICY sessoes_select_self ON public.tecnico_sessoes
  FOR SELECT TO authenticated
  USING (empresa_id = public.get_my_empresa_id());

CREATE POLICY sessoes_insert_self ON public.tecnico_sessoes
...
CREATE POLICY sessoes_update_self ON public.tecnico_sessoes
...
BEGIN
  ...
  WHERE id = p_os_id AND deleted_at IS NULL;

  IF v_os_empresa IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'OS não encontrada');
  END IF;

  IF v_os_empresa <> v_empresa THEN
    RETURN jsonb_build_object('success', false, 'error', 'OS de outra empresa');
  END IF;

  UPDATE public.ordens_de_servico
  SET tecnico_responsavel_id = v_funcionario_id,
      updated_at = now()
  WHERE id = p_os_id;
...
BEGIN
  ...
  SELECT to_jsonb(s.*) INTO v_sessao
  FROM public.tecnico_sessoes s
  WHERE s.funcionario_id = v_funcionario_id
