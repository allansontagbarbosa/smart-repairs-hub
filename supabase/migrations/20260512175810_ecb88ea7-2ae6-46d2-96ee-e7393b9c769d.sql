CREATE OR REPLACE FUNCTION public.gerar_folha_mensal_completa(
  p_competencia text,
  p_dia_vencimento int DEFAULT 5
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
  ...
  LOOP
    ...
    IF v_func.salario_centavos IS NOT NULL AND v_func.salario_centavos > 0 THEN
      IF NOT EXISTS (
        ...
        WHERE empresa_id = v_empresa_id
          ...
          AND descricao = 'SALÁRIO ' || upper(v_func.nome)
          AND deleted_at IS NULL
      ) THEN
    ...
    IF v_func.vt_centavos > 0 THEN
      IF NOT EXISTS (
        ...
        WHERE empresa_id = v_empresa_id
          AND mes_competencia = p_competencia
          AND descricao = 'VT ' || upper(v_func.nome)
          AND deleted_at IS NULL
      ) THEN
        INSERT INTO contas_a_pagar (
          empresa_id, descricao, valor, categoria, centro_custo,
    ...
    IF v_func.va_centavos > 0 THEN
      IF NOT EXISTS (
        ...
        WHERE empresa_id = v_empresa_id
          AND mes_competencia = p_competencia
          AND descricao = 'VA ' || upper(v_func.nome)
          AND deleted_at IS NULL
      ) THEN
        INSERT INTO contas_a_pagar (
          empresa_id, descricao, valor, categoria, centro_custo,
...
GRANT EXECUTE ON FUNCTION public.gerar_folha_mensal_completa(text, int) TO authenticated;