CREATE OR REPLACE FUNCTION public.get_painel_socio_v1(p_meses_historico integer DEFAULT 6)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- BUG 1 FIX: despesas (gastos fixos) são MENSAIS, não proporcionais.
  -- Usar valor do mês INTEIRO mesmo no card "parcial".
  ...
  v_despesas_parcial := v_despesas_completo;

  -- Recalcular LL, distribuível e meu_valor consistentes com despesas completas
  v_ll_parcial := v_fat_parcial - v_peca_parcial - v_com_parcial - v_despesas_parcial;
  v_distrib_parcial := CASE WHEN v_ll_parcial > 0 THEN v_ll_parcial * (100 - v_reserva_pct) / 100 ELSE 0 END;
  v_reserva_val_parcial := CASE WHEN v_ll_parcial > 0 THEN v_ll_parcial * v_reserva_pct / 100 ELSE 0 END;
  ...
  -- BUG 2 FIX: ocultar comparação quando OS concluídas no período do mês passado < 30
  SELECT COUNT(*) INTO v_qtd_os_periodo_mp
  FROM ordens_de_servico
  WHERE empresa_id = v_empresa_id AND deleted_at IS NULL
    AND data_conclusao >= v_inicio_mes_passado
    AND data_conclusao <= v_fim_periodo_mp;

  IF v_qtd_os_periodo_mp < 30 THEN
    v_meu_valor_periodo_mp := 0;
  END IF;
  ...
  -- Lista de sócios baseada no distribuível REAL (com despesas corretas)
  WITH socios_ativos AS (
    SELECT id, nome, COALESCE(percentual_participacao, 0) AS percentual
    FROM socios
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    'id', sa.id, 'nome', sa.nome, 'percentual', sa.percentual,
    'valor', round((v_distrib_parcial * sa.percentual / 100)::numeric, 2),
    'valor_estimado', round((v_distrib_parcial * sa.percentual / 100)::numeric, 2),
    'valor_mes_passado', COALESCE(mp.valor_mp, 0),
    'valor_ano_acumulado', COALESCE(ytd.valor_ytd, 0),
    'variacao_pct', CASE
      WHEN COALESCE(mp.valor_mp, 0) > 0 THEN round((((v_distrib_parcial * sa.percentual / 100) - mp.valor_mp) / mp.valor_mp * 100)::numeric, 1)
      WHEN (v_distrib_parcial * sa.percentual / 100) > 0 THEN 100 ELSE 0 END,
    'eh_voce', sa.id = v_socio_id
  )), '[]'::jsonb)
  INTO v_socios_lista
  FROM socios_ativos sa LEFT JOIN mp ON mp.id = sa.id LEFT JOIN ytd ON ytd.id = sa.id;
  ...
  v_resultado := jsonb_build_object(
    ...
    'mes_passado', jsonb_build_object(
      ...
      'qtd_os_periodo', v_qtd_os_periodo_mp
    ),
    'variacao_mes', jsonb_build_object(
      'meu_valor_pct', CASE WHEN v_meu_valor_periodo_mp > 0 THEN round(((v_meu_valor_parcial - v_meu_valor_periodo_mp) / v_meu_valor_periodo_mp * 100)::numeric, 1) WHEN v_meu_valor_parcial > 0 AND v_meu_valor_periodo_mp > 0 THEN 100 ELSE 0 END,
