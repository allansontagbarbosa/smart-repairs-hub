...
import { Search, Scan, X, Zap, Users, FileText, Maximize2, Store, ShoppingCart } from "lucide-react";
...
export default function LojaPDV() {
  ...
  const handleFinalizar = () => {
    alert("Finalizar venda — implementação na próxima fase");
  };

  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "F1") { e.preventDefault(); document.getElementById("pdv-search")?.focus(); }
      if (e.key === "F4" && carrinho.length > 0) { e.preventDefault(); handleFinalizar(); }
    };
    ...
    <div className="container mx-auto px-4 py-6 sm:py-8 max-w-7xl">
      <div className="mb-6 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" size="sm"><Users className="h-4 w-4 mr-2" /> Cliente F2</Button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-6">
        ...
        <div className="bg-card border rounded-xl p-4 h-fit lg:sticky lg:top-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Badge variant="outline">#V-{Date.now().toString().slice(-6)}</Badge>
          </div>
          ...
          {carrinho.length === 0 ? (
            ...
            <>
              ...
              <Button onClick={handleFinalizar} className="w-full h-12 text-base font-semibold">
