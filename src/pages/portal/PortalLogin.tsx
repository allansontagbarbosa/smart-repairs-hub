...
import { Wrench, Mail, Lock, Eye, EyeOff, Search, Hash } from "lucide-react";
...
export default function PortalLogin() {
  ...
  const [mode, setMode] = useState<"search" | "login">(searchParams.get("os") ? "search" : "search");
          ...
          {mode === "search" ? (
            <>
              ...
              {searchDone && searchResults.length === 0 && (
                <div className="text-center py-4">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Verifique os dados ou faça login para ver suas OS.</p>
                </div>
              )}
            ...
            <>
              ...
              <div className="text-center space-y-2">
                ...
                <div>
                  <button onClick={() => { setIsSignUp(!isSignUp); setIsForgot(false); }} className="text-xs text-primary hover:underline">
                    {isSignUp ? "Já tenho conta → Entrar" : "Não tenho conta → Criar"}
                  </button>
                </div>
                {isForgot && (
                  <button onClick={() => setIsForgot(false)} className="text-xs text-primary hover:underline">
                    ← Voltar ao login
                  </button>
                )}
                <div>
                  <button onClick={() => setMode("search")} className="text-xs text-muted-foreground hover:text-foreground">
