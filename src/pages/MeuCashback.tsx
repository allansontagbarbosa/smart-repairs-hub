...
export default function MeuCashback() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.rpc("get_meu_cashback");
          ...
          <CardContent className="pt-6 space-y-2">
            ...
            {data?.regra_ativa && (
              <div className="pt-2 border-t">
                <p className="text-xs text-muted-foreground">Sua taxa atual</p>
                <p className="font-semibold">
                  {data.regra_ativa.percentual != null
                    ? `${data.regra_ativa.percentual}% por OS`
                    : `${fmtC(data.regra_ativa.valor_fixo_centavos)} por OS`}
                </p>
              </div>
            )}
          </CardContent>
